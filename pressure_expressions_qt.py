...
def P_article(t, q, pd, pnd, delay = 0):
    if t < delay:
        return 0.0
    else :
        P0barre = q*pd.eta_visc/(4*np.pi*pd.fluid_density*pd.r_real*pd.k_perm*pd.sigma_n0)
        return P0barre * math.erfc(pnd.r / (2*math.sqrt(pnd.c * (t-delay))))

def dP_article(t, q, pd,pnd, delay = 0):
    P0barre = q*pd.eta_visc/(4*np.pi*pd.fluid_density*pd.r_real*pd.k_perm*pd.sigma_n0)
    if t < delay:
        return 0.0
    else :
...
# No pressure (P=0)
def P_none(t, q, pd, pnd, delay = find_cycle_start(path_without_pressure, taux)):
    return 0.0
def dP_none(t, pd, q, pnd, delay = find_cycle_start(path_without_pressure, taux)):
    return 0.0